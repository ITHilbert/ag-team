...
### Status & Verfügbarkeit (File-Based)
- Status ist **persistente Information** in `ag-team/status/`.
- Jeder Agent schreibt bei Statusänderung seine eigene Statusdatei: `ag-team/status/<ID>_<Rolle>.md`.
- **Format**:
  ```markdown
  # Agent Status
  Role: <Rolle>
  Status: BOOTING | READY | BUSY | ERROR
  Current_Task: <Kurzbeschreibung>
  Conversation_ID: <ID des aktuellen Chats>
  Last_Update: <Zeitstempel/Step>
  ```
- Chat-Nachrichten über Status ("...ist initialisiert") entfallen.
- Kommunikation prüft **nur Dateien**.
...
2. **Moderator** – Moderation, Klärung, Kapazitäts- & Entscheidungsinterface  
3. **Protokollagent** – Zentrale Anforderungssammlungen  
4. **Architekt** – Logikprüfung, Architekturentwurf, Tasking  
